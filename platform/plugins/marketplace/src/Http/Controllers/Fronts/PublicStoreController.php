...
class PublicStoreController extends BaseController
{
    ...
    public function getStore(
        ...
        if ($slug->key === 'tyson') {
            SeoHelper::setTitle($store->name . ' — 品牌落地页')
                ->setDescription($store->description ?: '优选蛋白产品，助力家庭美味与健康。');

            return Theme::scope(
                'marketplace.store-landing',
                compact('store', 'products'),
                MarketplaceHelper::viewPath('store-landing', false)
            )->render();
        }
