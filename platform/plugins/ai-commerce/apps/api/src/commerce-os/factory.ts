...
export function createCommerceOS(): CommerceOS {
  if (REAL_DATA_ONLY) {
    throw new Error('REAL_DATA_ONLY active — mock commerce OS runtime is disabled');
  }
  ...
  const retailLibrary = new RetailActionLibrary();

  // 4. Executor Components (New Architecture)
  const stepResolver = new StepResolver(themeActionLibrary, productLibrary, campaignLibrary, retailLibrary);
