...
function PreviewContent() {
  ...
  const router = useRouter();
   ...
   const templateId = searchParams?.get("templateId") || "preview";
   ...
   // REAL_DATA_ONLY: Minimal fallback structure (values should come from API or query params)
   const fallbackTemplate = {
     ...
     textColor: "#0F172A",
     speech: {
       userQuestion: "有什么推荐的新品吗？",
       aiAnswer: "欢迎来到我们的店铺！我们有最新的产品推荐。"
     },
     schema: {
       sections: {
         announcement: { text: "🔥 商店预览 · 实时数据接入" },
         header: { title: "商店名称" },
         hero: { title: "欢迎来到我们的商店", subtitle: "探索优质商品" },
         featured_collection: { title: "热销商品" },
         expert_tips: { title: "商店信息", desc: "通过统一平台数据库获取最新商品和优惠。" },
         footer: { copyright: "© 2026 模搭商用 SaaS" }
       }
     }
   };

   // Colors - all values come from query params, never from mock template
   const colors = {
     primary: overridePrimary || fallbackTemplate.primaryColor,
     secondary: overrideSecondary || fallbackTemplate.secondaryColor,
     bg: overrideBg || fallbackTemplate.bgColor,
     text: overrideText || fallbackTemplate.textColor
   };
  ...
  const [platformStats, setPlatformStats] = useState({ products: 0, orders: 0, customers: 0, discounts: 0 });
   ...
   // REAL_DATA_ONLY: Never fallback to template.products
   if (platformProducts.length === 0) {
     console.warn('⚠️ REAL_DATA_ONLY: No platform products available - blocking template fallback');
   }
   const displayProducts = platformProducts.map((item) => ({
