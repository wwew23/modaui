...
export function OrdersView({ orders, setOrders }: OrdersViewProps) {
    ...
    try {
      if (action.id === "auto-process") {
        ...
        if (pendingOrders.length > 0) {
          ...
          setAiResult(`已自动处理 ${pendingOrders.length} 笔待付款订单`);
        } else {
          setAiResult("没有待付款的订单");
        }
  ...
  const getStatusBadge = (status: Order['status']) => {
    const statusConfig = {
      pending: { label: '待付款', color: 'bg-amber-500/10 text-amber-600 border-amber-200', icon: Clock },
      processing: { label: '待发货', color: 'bg-blue-500/10 text-blue-600 border-blue-200', icon: Package },
      shipped: { label: '待收货', color: 'bg-foreground/10 text-foreground border-foreground/20', icon: Truck },
      delivered: { label: '已完成', color: 'bg-emerald-500/10 text-emerald-600 border-emerald-200', icon: CheckCircle },
        ...
        <div className="max-w-7xl space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              ...
              { key: 'pending', label: '待付款', count: statusCounts.pending },
              { key: 'processing', label: '待发货', count: statusCounts.processing },
              { key: 'shipped', label: '待收货', count: statusCounts.shipped },
              { key: 'delivered', label: '已完成', count: statusCounts.delivered },
