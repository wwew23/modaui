...
import { Users, Bot, FileText, TrendingUp, ArrowUpRight, Bell, Check, X, AlertTriangle, Zap } from 'lucide-react';
...
interface DashboardViewProps {
  ...
  onNavigate?: (tab: 'dashboard' | 'merchants' | 'stores' | 'agents' | 'queues' | 'logs' | 'billing' | 'analytics' | 'settings' | 'retail') => void;
}
...
export function DashboardView({ 
  ...
  const stats = [
    { label: '活跃商户', value: activeMerchants, total: merchants.length, icon: Users, trend: merchants.length > 0 ? '+0%' : '无数据' },
    { label: '在线助理', value: activeAgents, total: agents.length, icon: Bot, trend: agents.length > 0 ? '运行中' : '无数据' },
    { label: '今日操作', value: successLogs, total: logs.length, icon: FileText, trend: logs.length > 0 ? `${Math.round((successLogs / Math.max(logs.length, 1)) * 100)}%` : '0%' },
    { label: '本月增长', value: '0.0', unit: '%', icon: TrendingUp, trend: '+0.0%' },
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {merchants.length === 0 && agents.length === 0 ? (
        <div className="p-12 border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center text-center space-y-4">
          <div className="h-16 w-16 rounded-full bg-secondary flex items-center justify-center">
            <Zap className="h-8 w-8 text-muted-foreground animate-pulse" />
          </div>
          <div className="space-y-1">
            <h2 className="text-xl font-semibold">欢迎使用 CommerceOS</h2>
            <p className="text-muted-foreground max-w-sm">当前数据库为空，请先在商户管理中创建您的首个租户商户以开始业务运行。</p>
          </div>
          <button
            onClick={() => {
              ...
              if (typeof window !== 'undefined') {
                ...
                url.searchParams.set('tab', 'merchants');
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-lg bg-card border border-border overflow-hidden">
              ...
              <div className="divide-y divide-border">
                {merchants.length > 0 ? merchants.slice(0, 5).map((merchant) => (
            ...
            <div className="rounded-lg bg-card border border-border overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                {agents.length > 0 ? agents.map((agent) => (
                  ...
                  <div className="col-span-2 p-4 text-center text-sm text-muted-foreground italic">
                    暂无 AI 助理配置
                  </div>
                )}
              </div>
