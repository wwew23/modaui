        ...
        {agents.length > 0 ? agents.map((agent) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border">
              ...
              <button className="p-2 rounded-md bg-secondary text-foreground hover:bg-secondary/80 transition-colors" title="刷新状态">
                <RefreshCw className="h-4 w-4" />
              </button>
            </div>
          </div>
        )) : (
          <div className="col-span-3 p-12 border border-dashed border-border rounded-lg flex flex-col items-center justify-center text-center space-y-4">
            <div className="h-12 w-12 rounded-full bg-secondary flex items-center justify-center">
              <Bot className="h-6 w-6 text-muted-foreground" />
            </div>
            <div className="space-y-1">
              <h3 className="font-medium">暂无智能助理</h3>
              <p className="text-xs text-muted-foreground">当前运行时尚未部署任何 AI 助理，请检查系统配置或联系管理员。</p>
            </div>
          </div>
        )}
      </div>
