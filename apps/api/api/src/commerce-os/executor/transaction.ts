...
export class TransactionEngine {
  ...
  async begin(id: string): Promise<TransactionSession> {
    ...
    // Shopify Snapshots (Skip if not configured)
    if (process.env.SHOPIFY_STORE && process.env.SHOPIFY_ADMIN_TOKEN) {
      ...
      console.warn('[TransactionEngine] Shopify not configured, skipping snapshots');
    }
  ...
  async rollback(id: string) {
    ...
    if (session.snapshots.has('shopify.theme')) {
      rollbacks.push(this.shopifyThemeRuntime.rollback(session.snapshots.get('shopify.theme').id));
    }
    if (session.snapshots.has('shopify.product')) {
      rollbacks.push(this.shopifyProductRuntime.rollback(session.snapshots.get('shopify.product').id));
    }
    if (session.snapshots.has('shopify.campaign')) {
      rollbacks.push(this.shopifyCampaignRuntime.rollback(session.snapshots.get('shopify.campaign').id));
    }
