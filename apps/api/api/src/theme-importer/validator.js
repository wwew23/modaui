...
function scanForHtmlStrings(obj) {
  const findings = []
  function walk(o, p) {
    if (o == null) return
    if (typeof o === 'string') {
...
function validateTemplate(template) {
  ...
  if (!valid) {
    for (const e of validate.errors || []) {
      errors.push({ type: 'INVALID_SCHEMA', section: null, file: null, line: null, field: e.instancePath || e.dataPath || '', message: e.message, severity: 'fatal' })
    }
  }

  if (Array.isArray(template.content)) {
    template.content.forEach((c, idx) => {
...
function autoFixTemplate(template) {
  const fixes = []
...
module.exports = { validateTemplate, normalizeTemplate, autoFixTemplate }
