...
const assistantPersonas: Record<string, string> = {
  dashboard: "AI 总经理",
  products: "李设计师",
  customers: "赵客服主管",
  marketing: "张营销总监",
...
function generateAIResponse(input: string, context: string): string {
  const persona = assistantPersonas[context] || "AI 助手"

  if (input.includes("销售") || input.includes("报告")) {
    return `${persona}建议：根据您的需求，我可以帮您生成详细的销售报告。点击下方按钮开始执行。`
  }
  if (input.includes("发货") || input.includes("订单")) {
    return `${persona}建议：检测到 12 笔待发货订单。我可以帮您批量处理，自动生成发货单并通知客户。`
  }
  if (input.includes("客户") || input.includes("分析")) {
    return `${persona}建议：我可以帮您分析客户数据，识别高价值客户和流失风险客户，并给出精准营销建议。`
  }
  return `${persona}：好的，我理解您的需求是「${input}」。\n\n我可以帮您执行以下相关操作：`
}
