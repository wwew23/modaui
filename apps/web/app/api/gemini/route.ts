...
Example: If user typed "赛博朋克限量服装", prediction could be "潮玩阁，支持3D视觉".`;
      ...
      try {
        ...
        let fallback = "";
        const lowerInput = userTyped.toLowerCase();
        if (lowerInput.includes("原木") || lowerInput.includes("极简") || lowerInput.includes("咖啡")) {
          fallback = "，搭配3D实景和智能配方系统";
        } else if (lowerInput.includes("服饰") || lowerInput.includes("服装") || lowerInput.includes("杭派")) {
          fallback = "一手批发城，内置多模态AI识图";
        } else if (lowerInput.includes("数码") || lowerInput.includes("智能") || lowerInput.includes("电器")) {
          fallback = "百货生活馆，支持IoT智能说明书";
        } else if (lowerInput.includes("和牛") || lowerInput.includes("烧肉") || lowerInput.includes("美食") || lowerInput.includes("餐馆")) {
          fallback = "私房料理店，完美匹配高奢美学";
        } else if (lowerInput.includes("美容") || lowerInput.includes("spa") || lowerInput.includes("皮肤")) {
          fallback = "芳香美学体验中心，实时AI诊断";
        } else if (userTyped.length > 2) {
          fallback = "，定制3D仿真和AI客服";
        }
        return NextResponse.json({ success: true, prediction: fallback, fallback: true });
      }
    }
...
Do not write markdown code blocks or extra text. Return only valid stringified JSON.`;
      ...
      try {
        ...
        // Safe robust model fallback for instant performance stability
        const fallbackTitle = deconstructedData.title || `${cleanedDomain.charAt(0).toUpperCase() + cleanedDomain.slice(1)}`;
        const simulatedClone = {
          siteTitle: `${fallbackTitle} Premium Layout`,
          heroText: `Next-Generation Layout Inspired by ${cleanedDomain}`,
          heroSub: "Deconstructed design DNA structured beautifully with customized responsive parameters.",
          primaryColor: "#09090b",
          ...
          layoutMode: "cyber",
          clonedNodes: [
            { id: "nav", title: "Adaptive Sticky Header", content: "Platform, Dev Tools, Pricing Node, GitHub Profile" },
            { id: "hero", title: "Cloned Focal Showcase Banner", content: "High-density action button controls with custom gradient" },
            { id: "features", title: "Asynchronous Feature Blocks", "content": "3 key layout panels highlighting custom developer templates" },
            { id: "footer", title: "Global Workspace Footer", content: "System monitors, status nodes, copyright disclosures" }
          ]
        };
        return NextResponse.json({ success: true, cloneData: simulatedClone, fallback: true, warning: err.message });
      }
    }
