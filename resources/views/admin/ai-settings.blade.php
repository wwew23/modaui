...
@section('content')
    <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; padding: 24px; box-shadow: 0 12px 32px rgba(15, 23, 42, 0.08);">
            ...
            <div style="display: flex; align-items: center; gap: 12px; background: #f1f5f9; padding: 8px 16px; border-radius: 12px;">
                ...
                <select onchange="window.location.href='?shop_id=' + this.value" style="padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: #fff; font-size: 0.9rem; min-width: 200px;">
                    ...
                    @foreach($allShops as $s)
                        <option value="{{ $s->id }}" {{ ($shop && $shop->id == $s->id) ? 'selected' : '' }}>{{ $s->shop_domain }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <form action="{{ route('ai.settings.save') }}" method="POST">
            @csrf
            @if($shop)
                ...
                <section>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; background: #f8fafc; padding: 20px; border-radius: 14px;">
                        <label style="display: flex; align-items: center; gap: 12px; cursor: pointer;">
                            <input type="checkbox" name="enabled" value="1" {{ ($configs['ai_enabled'] ?? false) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                            <span style="font-weight: 500;">全局启用 AI</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 12px; cursor: pointer;">
                            <input type="checkbox" name="customer_agent_enabled" value="1" {{ ($configs['customer_agent_enabled'] ?? false) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                            <span style="font-weight: 500;">启用顾客导购</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 12px; cursor: pointer;">
                            <input type="checkbox" name="merchant_agent_enabled" value="1" {{ ($configs['merchant_agent_enabled'] ?? false) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                        ...
                        <div>
                            ...
                            <div style="display: flex; gap: 20px;">
                                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                    <input type="radio" name="model_source" value="openai" {{ ($configs['model_source'] ?? 'openai') === 'openai' ? 'checked' : '' }}>
                                    <span>OpenAI</span>
                                </label>
                                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                    <input type="radio" name="model_source" value="claude" {{ ($configs['model_source'] ?? '') === 'claude' ? 'checked' : '' }}>
                                    <span>Claude (Anthropic)</span>
                                </label>
                            </div>
                        </div>

                        <div id="panel-openai" style="display: {{ ($configs['model_source'] ?? 'openai') === 'openai' ? 'grid' : 'none' }}; gap: 16px;">
                            <div style="display: grid; gap: 8px;">
                                <label style="font-size: 0.9rem; font-weight: 500;">OpenAI API Key</label>
                                <input type="password" name="openai_api_key" value="{{ $configs['openai_api_key'] ?? '' }}" placeholder="sk-..." style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                            </div>
                            <div style="display: grid; gap: 8px;">
                                ...
                                <select name="openai_model" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                                    ...
                                    <option value="gpt-4-turbo" {{ ($configs['openai_model'] ?? '') === 'gpt-4-turbo' ? 'selected' : '' }}>gpt-4-turbo</option>
                                    <option value="gpt-3.5-turbo" {{ ($configs['openai_model'] ?? '') === 'gpt-3.5-turbo' ? 'selected' : '' }}>gpt-3.5-turbo</option>
                                </select>
                            </div>
                        </div>

                        <div id="panel-claude" style="display: {{ ($configs['model_source'] ?? '') === 'claude' ? 'grid' : 'none' }}; gap: 16px;">
                            <div style="display: grid; gap: 8px;">
                                <label style="font-size: 0.9rem; font-weight: 500;">Claude API Key</label>
                                <input type="password" name="claude_api_key" value="{{ $configs['claude_api_key'] ?? '' }}" placeholder="sk-ant-..." style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                            </div>
                            <div style="display: grid; gap: 8px;">
                                <label style="font-size: 0.9rem; font-weight: 500;">模型名称</label>
                                <select name="claude_model" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                                    <option value="claude-3-5-sonnet-20240620" {{ ($configs['claude_model'] ?? '') === 'claude-3-5-sonnet-20240620' ? 'selected' : '' }}>Claude 3.5 Sonnet</option>
                                    <option value="claude-3-opus-20240229" {{ ($configs['claude_model'] ?? '') === 'claude-3-opus-20240229' ? 'selected' : '' }}>Claude 3 Opus</option>
                                    <option value="claude-3-haiku-20240307" {{ ($configs['claude_model'] ?? '') === 'claude-3-haiku-20240307' ? 'selected' : '' }}>Claude 3 Haiku</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div style="border-top: 1px solid #e2e8f0; padding-top: 16px; margin-top: 8px;">
                            ...
                            <div style="display: grid; gap: 16px;">
                                <div style="display: grid; gap: 8px;">
                                    ...
                                    <input type="text" name="martfury_api_url" value="{{ $configs['martfury_api_url'] ?? 'https://modaui.com/api' }}" placeholder="https://your-domain.com/api" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                                </div>
                                <div style="display: grid; gap: 8px;">
                                    <label style="font-size: 0.9rem; font-weight: 500;">API Key / Token</label>
                                    <input type="password" name="martfury_api_key" value="{{ $configs['martfury_api_key'] ?? '' }}" placeholder="Enter API key" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; gap: 20px;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div style="display: grid; gap: 8px;">
                                ...
                                <input type="text" name="agent_tone" value="{{ $configs['agent_tone'] ?? '专业、简洁、富有洞察力' }}" placeholder="例如：亲切友好、幽默、严肃专业" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                            </div>
                            <div style="display: grid; gap: 8px;">
                                <label style="font-size: 0.9rem; font-weight: 500;">目标客群 (Target Audience)</label>
                                <input type="text" name="agent_audience" value="{{ $configs['agent_audience'] ?? '所有进店顾客' }}" placeholder="例如：18-25岁大学生、白领女性" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                            </div>
                        </div>

                        <div style="display: grid; gap: 8px;">
                            <label style="font-size: 0.9rem; font-weight: 500;">核心运营目标 (Priority Goals)</label>
                            <input type="text" name="agent_goals" value="{{ $configs['agent_goals'] ?? '提高客单价, 清理库存' }}" placeholder="多个目标请用逗号隔开" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                        </div>

                        <div style="display: grid; gap: 8px;">
                            <label style="font-size: 0.9rem; font-weight: 500;">禁用词/禁忌话题 (Forbidden Topics)</label>
                            <textarea name="agent_forbidden" rows="2" placeholder="例如：不要提到最低价、不要承诺不可能实现的效果" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; font-family: inherit;">{{ $configs['agent_forbidden'] ?? '' }}</textarea>
                        </div>

                        <div style="display: grid; gap: 8px;">
                            <label style="font-size: 0.9rem; font-weight: 500;">商家侧系统提示词 (Merchant System Prompt)</label>
                            <textarea name="merchant_prompt" rows="4" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; font-family: inherit;">{{ $configs['merchant_prompt'] ?? '' }}</textarea>
                        </div>
                    </div>
                </section>
                    ...
                    <div style="display: grid; gap: 20px;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div style="display: grid; gap: 8px;">
                                ...
                                <div style="display: flex; gap: 10px; align-items: center;">
                                    <input type="color" name="frontend_primary_color" value="{{ $configs['frontend_primary_color'] ?? '#008060' }}" style="width: 44px; height: 44px; padding: 0; border: 1px solid #e2e8f0; border-radius: 8px; cursor: pointer;">
                                    <input type="text" value="{{ $configs['frontend_primary_color'] ?? '#008060' }}" readonly style="flex: 1; padding: 10px; border: 1px solid #e2e8f0; border-radius: 10px; background: #f8fafc; color: #64748b; font-family: monospace;">
                                </div>
                            </div>
                            <div style="display: grid; gap: 8px;">
                                <label style="font-size: 0.9rem; font-weight: 500;">入口悬浮文案</label>
                                <input type="text" name="frontend_floating_text" value="{{ $configs['frontend_floating_text'] ?? '有问题？问 AI 导购' }}" placeholder="例如：有问题？问 AI 导购" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                            </div>
                        </div>

                        <div style="display: grid; gap: 8px;">
                            <label style="font-size: 0.9rem; font-weight: 500;">初始欢迎语 (Welcome Message)</label>
                            <textarea name="frontend_welcome_message" rows="2" placeholder="例如：嗨！我是您的购物助手，我可以帮您找商品、查订单..." style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">{{ $configs['frontend_welcome_message'] ?? '' }}</textarea>
                        </div>
                        ...
                        <div style="display: grid; gap: 8px;">
                            ...
                            <select name="frontend_tone" style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px;">
                                <option value="friendly" {{ ($configs['frontend_tone'] ?? '') === 'friendly' ? 'selected' : '' }}>亲切友好 (推荐)</option>
                                <option value="professional" {{ ($configs['frontend_tone'] ?? '') === 'professional' ? 'selected' : '' }}>专业严谨</option>
                                <option value="humorous" {{ ($configs['frontend_tone'] ?? '') === 'humorous' ? 'selected' : '' }}>风趣幽默</option>
                                <option value="minimalist" {{ ($configs['frontend_tone'] ?? '') === 'minimalist' ? 'selected' : '' }}>极简直接</option>
                            </select>
                        </div>
                    </div>
                </section>
