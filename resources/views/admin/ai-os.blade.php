...
@section('title', 'AI Commerce OS 控制台')
    ...
    <div style="display: grid; gap: 24px;">
        <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; padding: 20px; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.06);">
            <div style="display: flex; flex-wrap: wrap; gap: 12px; justify-content: space-between; align-items: center;">
                ...
                <a href="{{ route('admin.dashboard') }}" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: 12px; background: #111827; color: #fff; text-decoration: none; font-weight: 600;">← 返回后台</a>
            </div>
        ...
        <div id="ai-os-console-panel" style="background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden; min-height: 520px; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.06);">
            ...
            <iframe src="{{ route('ai-commerce.dashboard') }}" style="width: 100%; height: 680px; border: none;"></iframe>
        </div>
            ...
            <div style="margin-top: 20px; display: grid; gap: 18px;">
                <div style="display: grid; grid-template-columns: repeat(2, minmax(200px, 1fr)); gap: 16px;">
                    <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                        ...
                        <p style="margin-top: 8px; color: #111827;">{{ $aiEnabled ? '已启用' : '已禁用' }}</p>
                    </div>
                    <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                        <strong>客户端智能体</strong>
                        <p style="margin-top: 8px; color: #111827;">{{ $customerAgentEnabled ? '启用' : '禁用' }}</p>
                    </div>
                    <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                        <strong>商家智能体</strong>
                        <p style="margin-top: 8px; color: #111827;">{{ $merchantAgentEnabled ? '启用' : '禁用' }}</p>
                    </div>
                    <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                        <strong>运行时来源</strong>
                        <p style="margin-top: 8px; color: #111827;">{{ strtoupper($modelSource) }}</p>
                    </div>
                </div>

                <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                    <strong>当前模型配置</strong>
                    <p style="margin-top: 8px; color: #111827; word-break: break-word;">{{ $modelEndpoint ?? '未配置' }}</p>
                    @if(in_array($modelSource, ['llama', 'ollama']))
                        <p style="margin-top: 8px; color: #6b7280;">模型名称：{{ $modelName ?? '默认' }}</p>
                        <p style="margin-top: 8px; color: #6b7280;">温度：{{ $modelTemperature ?? '默认' }}, 最大 Tokens：{{ $modelMaxTokens ?? '默认' }}</p>
                    @endif
                ...
                <div style="padding: 18px; border: 1px solid #e5e7eb; border-radius: 14px; background: #f8fafc;">
                    <strong>风格与行业</strong>
                    <p style="margin-top: 8px; color: #111827;">行业：{{ $industry ?? '未设置' }}</p>
                    <p style="margin-top: 8px; color: #111827;">风格：{{ $stylePreset ? ucfirst(str_replace('_', ' ', $stylePreset)) : '未设置' }}</p>
                </div>

                <div style="display: flex; gap: 12px; flex-wrap: wrap; margin-top: 8px;">
                    <a href="{{ route('ai.settings', ['shop_id' => 1]) }}" style="display: inline-block; padding: 12px 18px; border-radius: 12px; background: #111827; color: #fff; text-decoration: none;">前往 AI 设置页面</a>
                    <a href="{{ route('ai-commerce.dashboard') }}" style="display: inline-block; padding: 12px 18px; border-radius: 12px; background: #047857; color: #fff; text-decoration: none;">查看 AI 运行时控制台</a>
                </div>
            </div>
        </div>
        ...
        <div id="ai-os-assistant-panel" style="display: none; background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; padding: 20px; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.06);">
            ...
            <div style="margin-top: 18px; display: grid; gap: 14px;">
                <a href="{{ route('ai.assistant') }}" style="display: inline-block; padding: 12px 18px; border-radius: 12px; background: #047857; color: #fff; text-decoration: none;">打开 AI 运营助手页面</a>
                <a href="{{ route('ai-commerce.dashboard') }}" style="display: inline-block; padding: 12px 18px; border-radius: 12px; background: #111827; color: #fff; text-decoration: none;">打开 AI 智能中心（运行时控制台）</a>
            </div>
